...
def get_data(url):
    ...
    for i in range(0, len(a['Items'])):
        ...
        for j in range(0,len(a['Items'][i]['Lines'])):
            e["Account"].append({"Account_Name": a['Items'][i]['Lines'][j]['Account']['Name'], "Type": a['Items'][i]['Lines'][j]['Type'],"Total": a['Items'][i]['Lines'][j]['Total'],"Description":a['Items'][i]['Lines'][j]['Description']})
     
             
        arr.append(e)

        filename = "{}/professional.json".format(path)
        file_exists = exists("{}/professional.json".format(path))

        if file_exists == False:
            with open("{}/professional.json".format(path), "w") as jsonFile:
