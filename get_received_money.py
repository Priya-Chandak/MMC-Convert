...
url = "https://arl2.api.myob.com/accountright/53f60d69-ae83-4722-99a8-bdfc30d65040/Banking/ReceiveMoneyTxn/?$top={}".format(
    limit)
response = requests.request("GET", url, headers=headers, data=payload)
a = response.json()
...
def get_data(url):
    ...
    for i in range(0, len(a['Items'])):
        ...
        filename = "{}/received_money.json".format(path)
        file_exists = exists("{}/received_money.json".format(path))

        if file_exists == False:
            with open("{}/received_money.json".format(path), "w") as jsonFile:
